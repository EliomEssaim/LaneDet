...
def vis_one(results, filename, width=9,mode='gt_on'):
    # 增加了mode 表示不显示gt
    ...
    if mode=='gt_on': img_gt_pil = PIL.Image.fromarray(img_gt)
    num_failed = 0
    ...
    if mode == 'gt_on': img_gt = np.array(img_gt_pil, dtype=np.uint8)
...
def single_gpu_test(seg_model,
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            mkdir(dst_folder)
            seeds, hm = seg_model(return_loss=False, rescale=False, thr=hm_thr, **data)
